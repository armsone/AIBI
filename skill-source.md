...
**AIBI 0.5.1 maintenance note (Stargram only):** When Gemini or Claude lacks an enclosing form, search editor ancestors for a unique visible semantic send button; reject broad document/body/html queries and ambiguous or disabled candidates. Attachment preview fallback starts in the nearest shared ancestor of the current editor and unique semantic send button, rejecting history-containing scopes and document/body/html. Exact expected preview count remains unchanged, and selector-family counts are never summed across overlapping families. One-shot dispatch, cancellation, send ambiguity, and disabled guards are preserved, while common runtime and DenimDex remain unchanged. Android source is updated; physical-device behavior remains unverified.
