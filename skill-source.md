...
**AIBI 0.5.1 maintenance note (Stargram only):** When Gemini or Claude lacks an enclosing form, search editor ancestors for a unique visible semantic send button; reject broad document/body/html queries and ambiguous or disabled candidates. Attachment handling, one-shot dispatch, and cancellation guards are preserved, while common runtime and DenimDex remain unchanged. iPhone visible verification confirmed single send and result application for Gemini and Claude; Android source is updated; physical-device behavior remains unverified and build status belongs in the verification record.
