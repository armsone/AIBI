...
An AIBI integration is complete only when the target app delivers all applicable outcomes from the canonical portable contract: truthful login state and automatic login dismissal, hidden mode that remains hidden with host progress, visible mode from task start, user-action-only takeover, verified submission, stable final-result extraction, exactly-once host commit, an always-visible cancel action during execution, a finite post-submission timeout (portable default 119 seconds) shown as a `1:59 → 0:00` countdown and decreasing progress bar, safe cancellation/error handling, and provider drift isolation. Build success alone is never completion.
...
2. **Platform & Session Storage**:
   ...
   - Treat provider session status as observed state, not a remembered guess. Settings must distinguish `checking`, `authenticated`, and `login required` using positive page evidence; cookie presence and absence of a login button are not authentication proof.
...
- Preserve the proven login-status recipe when porting: use the same persistent browser store as the visible login surface; create one provider probe at a time; attach it to an Activity/window or UIKit view hierarchy at a real 375×667 viewport and move it off-screen without making it `GONE`; wait for main-frame navigation plus SPA hydration; evaluate provider-specific visible composer/account markers, visible login controls, and visible challenge markers; publish exactly one of `checking`, `authenticated`, or `login required`; destroy the probe before advancing. In the visible login surface, repeat the positive check after each allowed main-frame navigation and during bounded hydration polling; on positive evidence flush the browser store, report success once, cancel stale callbacks, and dismiss exactly once. Do not infer authentication from cookies, URL shape, absence of a login button, or hidden DOM templates.
